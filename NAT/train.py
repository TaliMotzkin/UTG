...
def train_val(train_val_data, model, mode, bs, epochs, criterion, optimizer, early_stopper, rand_samplers, logger, model_dim, n_hop=2):
  ...
  for epoch in range(epochs):
    ...
    for k in tqdm(range(num_batch)):
      ...
      if k < 2:
          print("src_l_cut", src_l_cut[:2], src_l_cut.shape)
          print("tgt_l_cut", tgt_l_cut[:2], tgt_l_cut.shape)
          print("bad_l_cut", bad_l_cut[:2], bad_l_cut.shape)
          print("ts_l_cut" , ts_l_cut[:2], ts_l_cut.shape)
          print("e_l_cut", e_l_cut[:2], e_l_cut.shape)
