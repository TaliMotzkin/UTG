        ...
        # query_src_indices = torch.randint( 0, size_batch, (size_cut,), dtype=torch.long, device=args.device)
            ...
            with torch.no_grad():
                ...
                if getattr(args, "with_hop", 0) == 1: #no need to calculate over the same batch again and again 
                    ...
                    pos_nat, _ = NAT_module.contrast(query_src[0].unsqueeze(0), query_dst[0].unsqueeze(0), query_dst[0].unsqueeze(0), time_snap[0].unsqueeze(0), idx_snap[0].unsqueeze(0), test=True)
                    _, neg_nat = NAT_module.contrast(query_src[1:], query_dst[1:], query_dst[1:], time_snap[1:], idx_snap[1:], test=True)
                    ...
                    # cat_fake = torch.cat([h[query_src], h[query_dst]], dim=0)
        ...
        for epoch in range(num_epochs):
            ...
            for snapshot_idx in range(train_data['time_length']): #207
                ...
                if args.with_hop ==1:
                    ...
                    pos_hop , neg_hop = modelN.contrast(pos_index[0], pos_index[1], neg_dst, time_snap, edge_snap, test =True)
                    # print("H", h.shape)
